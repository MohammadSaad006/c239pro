...
#         a where condition defined in the filter_by() function and the first() means that we
#           want to get just the first value, since we are expecting and needing only one result
#           here.
#           a. SQLAlchemy itself provides a secure layer where SQL Injection cannot be
#              performed on the database.

        user = Users.query.filter_by(email=email, password=password).first()

#         Here, session is used to save the email and user_id of the user if their credentials are
#         correct.
#         a. Sessions are browser sessions for when a user has logged in. It works like a
#            dictionary and can hold values for each and every user specific to their
#            browser.
#         b. This means that these email and user_id values can be retrieved for the user
#            who is accessing the website from the same browser again and again,
#            without them having to log back in again.
...
def submit_help():
    ...
    if attachment:
        ...
        # We use .png , .jpg , .jpeg , .gif extension to avoid uplod any other file becouse any uplode virus file in to website/app
